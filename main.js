...
<div class="wrap">
  ...
  <div class="layout">
    <div class="panel">
      ...
      <div id="subtitle" class="subtitle">No paid APIs. Simulation, memory, relationships and speech all run locally.</div>
    </div>
...
const canvas=$("world"),ctx=canvas.getContext("2d"),W=canvas.width,H=canvas.height;
const rnd=(a,b)=>a+Math.random()*(b-a),clamp=(v,a,b)=>Math.max(a,Math.min(b,v)),dist=(a,b)=>Math.hypot(a.x-b.x,a.y-b.y);
const chance=p=>Math.random()<p;
const pick=a=>a[Math.floor(Math.random()*a.length)];

let running=false,speed=1,selected=0,activeTab="overview",talkBusy=false,lastTalk=-999,tts=null,ttsLoading=false,currentAudio=null;
let world;
...
function remember(a,type,text,importance=1){
  a.memories.push({day:world.day,time:world.time,type,text,importance});
...
async function speak(name,text){
  ...
  try{
    ...
    const audio=await tts.generate(text,{voice:person.voice,speed:1});
    const blob=audio.toBlob?audio.toBlob():null;if(!blob)return;
...
async function conversation(lines,onDone){
  ...
  for(const [n,t] of lines){await speak(n,t);await new Promise(r=>setTimeout(r,120));}
  talkBusy=false;$("conversation").textContent="Conversation finished.";if(onDone)onDone();
}

function newWorld(){
  const resources=[];
  for(let i=0;i<86;i++)resources.push({type:"tree",x:rnd(25,W-25),y:rnd(25,H-25),amount:4});
  for(let i=0;i<26;i++)resources.push({type:"stone",x:rnd(25,W-25),y:rnd(25,H-25),amount:3});
  for(let i=0;i<34;i++)resources.push({type:"food",x:rnd(25,W-25),y:rnd(25,H-25),amount:2});
  for(let i=0;i<25;i++)resources.push({type:"fibre",x:rnd(25,W-25),y:rnd(25,H-25),amount:2});
  for(let i=0;i<16;i++)resources.push({type:"clay",x:rnd(25,W-25),y:rnd(25,H-25),amount:2});
  ...
  const agents=CHARACTERS.map((c,i)=>{
    ...
    return {...c,x:W*.22+i*27,y:H*.59+i*10,hunger:rnd(74,90),thirst:rnd(74,90),energy:rnd(78,95),warmth:rnd(72,91),social:rnd(55,82),
      ...
      wander:{x:rnd(30,W-30),y:rnd(30,H-30)},lastDiscovery:0,lastExperiment:0};
  });
...
async function contextualTalk(a,b){
  if(talkBusy)return;
  if(disagreement(a,b)){
    await conversation([[a.name,"I don’t think that is the right priority."],[b.name,"I disagree, but we can keep working and revisit it later."]]);
    return;
  }
  if(teach(a,b)&&chance(.7)){
    await conversation([[a.name,"I learned something useful. Let me show you."],[b.name,"Good. I’ll remember that."]]);
    return;
  }
  if(!world.fire){
    await conversation([[a.name,"Night is getting closer. We need timber for a fire."],[b.name,"Agreed. I’ll gather wood and bring it back to camp."]],()=>addPromise(b,a,"wood","gather firewood"));
  }else if(!world.shelter){
    await conversation([[a.name,"The fire helps, but we still need shelter."],[b.name,"I’ll collect more timber. Let’s finish it before the temperature drops."]],()=>addPromise(b,a,"wood","help build shelter"));
  }else if(world.food<8){
    await conversation([[a.name,"Our food reserve is getting low."],[b.name,"I’ll search for berries and bring back what I find."]],()=>addPromise(b,a,"food","gather food"));
  }else{
    await conversation([[a.name,a.role==="Explorer"?"I found useful resources farther out.":"The settlement is stable. We should keep building reserves."],[b.name,"Understood. I’ll keep that in mind while I work."]]);
  }
}
function act(a,i){
  ...
  if(world.tick%58===i*9)decide(a,i);
  ...
  if(a.action==="Walking to river"){if(move(a,a.target)){a.thirst=100;a.action="Drinking";remember(a,"need","Drank from the river",1);}}
    ...
    if(move(a,a.target)&&world.tick-lastTalk>260&&!talkBusy){lastTalk=world.tick;contextualTalk(a,a.target);}
  }else{
...
function render(){
  draw();$("day").textContent=world.day;$("time").textContent=String(Math.floor(world.time)).padStart(2,"0")+":"+String(Math.floor((world.time%1)*60)).padStart(2,"0");
